...
const highScoreTally = document.querySelector(".high-score");
...
highScoreTally.innerText = `High Score: ${highScore}`
...
const handleGameOver = () => {
    //clear the timer and reloading the page when its game over
...
controls.forEach(key => {
    ...
    key.addEventListener('click', () => changeDirection({key: let.dataset.key }))
})
...
const initGame = () => {
    ...
    snakeX += velocityX;
    snakeY += velocityY;
    //checking if the snake's head is put of wall, if so setting gameOver to true
    ...
    for(let i = 0; i < snakeBody.length; i++) {
        ...
        // Checking if the snake head hit the body; if so Gameover
